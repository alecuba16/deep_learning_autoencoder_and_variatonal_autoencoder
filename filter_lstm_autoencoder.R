filter_lstm_autoencoder<-function(model=NULL,wtdata=NULL,timesteps=model$timesteps,epochs=model$epochs,batch_size=model$batch_size,date_time_name='date_time',target_name='alarm',exclude_variables=model$exclude_variables,sdv=model$sdv,mean=model$mean,zero_sdv_columns=model$zero_sdv_columns,view_metrics=F,verbose=T,encode=model$encode,denormalize=model$denormalize,model_save_folder=model$model_save_folder,use_gpu=F){
  iam=match.call()[[1]]
  ...
  if(is.null(model)||is.na(model)){ #Create new model
    ...
    model_train <- keras_model(inputs,decoded)
    model_encoder <- keras_model(inputs,encoded)
    ...
    #Train
    if(is.null(model_save_folder)||is.na(model_save_folder)) model_save_folder<-'tmp'
    ...
    if(any(grepl(pattern = '/device:GPU',x = dl))&&use_gpu){
      with(tf$device("/device:GPU:0"), {
        history <- model_train %>% keras::fit(wtdata,wtdata,epochs = epochs,batch_size = batch_size,view_metrics=view_metrics,verbose=verbose)
      })
    }else{
    ...
    keras::save_model_hdf5(object=model_train,filepath=paste0(model_save_folder,'/model_ae_train.h5'),overwrite=T,include_optimizer=T)
    keras::save_model_hdf5(object=model_encoder,filepath=paste0(model_save_folder,'/model_ae_encoder.h5'),overwrite=T)
    
    model<-list(timesteps=timesteps,epochs=epochs,batch_size=batch_size,exclude_variables=exclude_variables,sdv=sdv,mean=mean,zero_sdv_columns=zero_sdv_columns,encode=encode,denormalize=denormalize,model_save_folder=model_save_folder,train_path=paste0(model_save_folder,'/model_ae_train.h5'),encoder_path=paste0(model_save_folder,'/model_ae_encoder.h5'),history=history)
  }else{
    model_train<-keras::load_model_hdf5(filepath=model$train_path,compile = TRUE,custom_objects = list('metric_rmse'=metric_rmse))
    model_encoder<-keras::load_model_hdf5(filepath=model$encoder_path,compile = TRUE)
  }
  
  if(encode){
    wtdata<-model_encoder %>% predict(wtdata)
    wtdata<-as.data.frame(wtdata)
  }else{
    #Reconstruct data
    wtdata<-model_train %>% predict(wtdata)
    wtdata<-as.data.frame(wtdata)
    if(denormalize){
      #De normalize
      for(c in 1:ncol(wtdata)){
  ...
  return(list(error=FALSE,data=list(model=model,wtdata=wtdata,selected_rows=selected_rows)))
}
